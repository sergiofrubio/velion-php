<?php
$pageTitle = "Modificar Cita - Velion";
...
<div class="max-w-3xl mx-auto animate-fade-in pb-12">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
        <form action="<?= PROJECT_ROOT ?>/vista-pacientes/citas/edit" method="POST" class="p-8 md:p-12">
                        ...
                        <div class="space-y-2" id="container-fisio">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="fisio_search" placeholder="Buscar profesional..." 
                                    value="<?= htmlspecialchars($appointment['fisioterapeuta_nombre'] . ' ' . $appointment['fisioterapeuta_apellidos']) ?>"
                                    class="block w-full rounded-2xl border-primary-100 shadow-sm focus:border-primary-500 focus:ring-primary-500 text-sm border p-4 pl-11 transition-all bg-primary-50/30" autocomplete="off" required>
                                <input type="hidden" name="fisioterapeuta_id" id="fisioterapeuta_id" value="<?= $appointment['fisioterapeuta_id'] ?>" required>
                                <div id="fisio_results" class="absolute z-20 w-full mt-2 bg-white rounded-2xl shadow-2xl border border-gray-100 hidden max-h-64 overflow-y-auto overflow-x-hidden py-2">
                                    <!-- Results -->
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="especialidad_id" id="especialidad_id" required
                                ...
                                <?php foreach ($especialidades as $e): ?>
                                    <option value="<?= $e['especialidad_id'] ?>" <?= $e['especialidad_id'] == $appointment['especialidad_id'] ? 'selected' : '' ?>><?= $e['descripcion'] ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2 md:col-span-2">
                            <label for="fecha_hora" class="block text-xs font-black text-gray-400 uppercase tracking-widest ml-1">Selecciona el nuevo momento</label>
                            <input type="datetime-local" name="fecha_hora" id="fecha_hora" value="<?= date('Y-m-d\TH:i', strtotime($appointment['fecha_hora'])) ?>" required
